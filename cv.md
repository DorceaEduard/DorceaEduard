# Dorcea Heia Eduard Damian

Automatică & Calculatoare, UTCB (anul III)
eduarddorcea@gmail.com · 0764 958 970
GitHub: [github.com/DorceaEduard](https://github.com/DorceaEduard) · LinkedIn: [linkedin.com/in/dorcea-heia-eduard-damian](https://www.linkedin.com/in/dorcea-heia-eduard-damian-781506333/)

---

Construiesc sisteme de cunoștințe AI. Recent: AIASSIST, un asistent RAG local care ingerează PDF-uri de curs și răspunde cu citare sursă, live pe Vercel.

---

## Proiecte

**AIASSIST — asistent RAG local pentru studenți** · [live](https://proiect-edi.vercel.app) · [cod](https://github.com/MihaiBlinzeanu/ProiectEdi-1)

- Am construit pipeline-ul de ingestie PDF: parsez automat 44 de cursuri (4 materii) în 371 fragmente, pregătite pentru indexare semantică
- Am implementat sistemul de notițe: extrag și structurez automat concepte din PDF-uri, creând un vault de cunoștințe interconectat
- Stack: Python, FAISS, NVIDIA NIM (embeddings + LLM), pdfplumber, Vercel. Răspunsuri cu citare sursă, anti-halucinație

---

## Experiență

**Practicant Dezvoltare Software — DeviDevs** (iunie-iulie 2026)
- Am construit AIASSIST, sistem RAG local: ingest PDF → chunking → embeddings → căutare semantică → răspuns cu citări
- Stack: Python, FAISS, NVIDIA NIM, pdfplumber, Vercel

**Project Manager Intern — EGIS Romania** (iunie 2025 - iulie 2026)
- Am gestionat proiecte folosind metodologia Agile (Scrum, Sprint Planning)
- Am lucrat cu Microsoft Power Platform: PowerApps, Power Automate, Power BI

---

## Skills

Limbaje: Python, JavaScript, TypeScript, C#
Web: Next.js, React, Node.js, Tailwind CSS
Data & AI: FAISS, NVIDIA NIM, RAG, PDF Processing, Supabase, PostgreSQL
Tools: Git, Vercel, Microsoft Power Platform (PowerApps, Power Automate, Power BI)

---

## Educație

Automatică & Calculatoare, UTCB — Anul III (2024-2027)